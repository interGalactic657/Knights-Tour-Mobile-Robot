...
def run_testbench(subdir, test_file, mode, debug_mode):
    ...
    if args.post_synthesis:
        ...
        sim_command = sim_command.extend([
                "vsim", 
                "-c", 
                "-do", 
    ...
    if debug_mode == 2:
        ...
        if mode == "cmd":
            sim_command.extend([ f"vsim -c -do \"" 
            ...
            if result == "success":
                ...
                if debug_mode == 0:
                    ...
                    subprocess.run(sim_command.extend([f"vsim -wlf {wave_file} work.KnightsTour_tb -voptargs=\"+acc\" -do \"{add_wave_command} run -all; write format wave -window .main_pane.wave.interior.cs.body.pw.wf {wave_format_file}; log -flush /*; quit -f;\""]),
                        shell=True, check=True
                    )   
                elif debug_mode == 1:
                    print(f"{test_name}: Test failed. Launching GUI for debugging...")
                    subprocess.run(sim_command.extend([f"vsim -wlf {wave_file} work.KnightsTour_tb -voptargs=\"+acc\" -do \"{add_wave_command} run -all; write format wave -window .main_pane.wave.interior.cs.body.pw.wf {wave_format_file}; log -flush /*;\""]),
                            shell=True, check=True
                    )
