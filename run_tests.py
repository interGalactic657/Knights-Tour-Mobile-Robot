    ...
    if mode == "cmd":
        ...
        if debug_mode == 0:
            ...
            # Your existing conditions
                ...
                default_transcript = os.path.join(waves_dir, "transcript")  # ModelSim default location

                # Define the new file name for the transcript
                new_transcript_name = f"{test_name}.log"
                new_transcript_path = os.path.join(transcript_dir, new_transcript_name)

                # Move and rename the transcript
                try:
                    shutil.move(default_transcript, new_transcript_path)
                    print(f"Moved and renamed transcript to {new_transcript_path}")
                except FileNotFoundError:
                    print("Transcript file not found, skipping move.")
                except Exception as e:
                    print(f"Error occurred while moving the transcript: {e}")

                # Check the transcript for success or error
                result = check_transcript(new_transcript_name)
